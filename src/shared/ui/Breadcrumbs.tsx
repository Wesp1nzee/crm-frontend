...
export function Breadcrumbs() {
  ...
  const clientIdIndex = pathnames.findIndex((segment, index) => segment === 'clients' && index < pathnames.length - 1);
      ...
      <MuiBreadcrumbs separator={<NavigateNext fontSize="small" />}>
        ...
        {breadcrumbsSegments.map((segment, index) => {
          ...
          return isLast ? (
            ...
            <Link key={fullPath} component={RouterLink} to={fullPath} color="inherit">
              {name}
            </Link>
          );
        })}
