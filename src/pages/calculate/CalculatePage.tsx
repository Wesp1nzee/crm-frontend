...
export function CalculatePage() {
  ...
  const isFormInvalid =
    !tableName.trim() || (selectedType === "leifer" && !selectedDictionary);
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
        Выберите тип расчета для создания новой таблицы или просмотра
        существующих
      </Typography>
        ...
        {calculationTypes.map((type) => (
          <Grid size={{ xs: 12, md: 6 }} key={type.id}>
            <CalculationCard
              ...
              completedCount={
                getFilteredTables(type.id).filter(
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Создать новую таблицу расчетов</DialogTitle>
        <DialogContent>
      ...
      >
                  ...
                  <ListItemText
                    primary={
                      <Box display="flex" alignItems="center" gap={1}>
                        ...
                        <Chip
                          ...
                          color={
                            table.status === "completed" ? "success" : "default"
                          }
                        />
                      </Box>
                    }
                    secondary={
                      <Box>
                        <Typography variant="body2" color="text.secondary">
                          Создано:{" "}
                          {dayjs(table.createdAt).format("DD.MM.YYYY HH:mm")}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Изменено:{" "}
                          {dayjs(table.lastModified).format("DD.MM.YYYY HH:mm")}
                        </Typography>
                      </Box>
                    }
                  />
        ...
        <DialogActions>
          <Button onClick={() => setListDialogOpen(false)}>Закрыть</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
