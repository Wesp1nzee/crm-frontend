...
const FileIcon = ({ contentType, size = 26 }: { contentType: string; size?: number }) => {
  ...
  if (contentType.startsWith("image/"))
    return <Image sx={{ ...s, color: "#10b981" }} />;
  if (contentType === "application/pdf")
    return <PictureAsPdf sx={{ ...s, color: "#ef4444" }} />;
  if (contentType.startsWith("video/"))
    return <VideoFile sx={{ ...s, color: "#8b5cf6" }} />;
  if (contentType.startsWith("audio/"))
    return <AudioFile sx={{ ...s, color: "#f59e0b" }} />;
  if (contentType.includes("zip") || contentType.includes("archive") || contentType.includes("rar"))
    return <FolderZip sx={{ ...s, color: "#f97316" }} />;
  if (contentType.includes("word") || contentType.includes("document"))
    return <Description sx={{ ...s, color: "#3b82f6" }} />;
  if (contentType.includes("sheet") || contentType.includes("excel"))
    return <TableChart sx={{ ...s, color: "#22c55e" }} />;
  if (contentType.includes("presentation") || contentType.includes("powerpoint"))
    return <Slideshow sx={{ ...s, color: "#f43f5e" }} />;
  if (contentType.includes("json") || contentType.includes("xml") || contentType.includes("html"))
    return <Code sx={{ ...s, color: "#06b6d4" }} />;
  if (contentType.startsWith("text/"))
    return <TextSnippet sx={{ ...s, color: "#94a3b8" }} />;
  return <InsertDriveFile sx={{ ...s, color: "#94a3b8" }} />;
};

const isPreviewable = (ct: string) =>
  ct.startsWith("image/") || ct === "application/pdf" || ct.startsWith("video/");
    ...
    >
      <Box
        ...
        sx={{
          ...
          minWidth: "320px",
          opacity: 0.08,
          filter: "drop-shadow(0 20px 45px rgba(99,102,241,0.18))",
        }}
      />
    </Box>
  );
  ...
  return (
    <Card
      ...
      sx={{
        ...
        background: hovered ? "linear-gradient(145deg,#fff,#f5f3ff)" : "#fff",
        boxShadow: hovered
          ? "0 20px 60px rgba(79,70,229,0.13),0 4px 16px rgba(0,0,0,0.06)"
    ...
    >
        ...
        <Stack direction="row" alignItems="flex-start" spacing={1.5}>
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            <Typography
              ...
              sx={{ wordBreak: "break-word", lineHeight: 1.35, fontSize: "0.85rem", color: "#0f172a" }}
            >
              {file.filename}
            </Typography>
      ...
      >
        {isPreviewable(file.content_type) && (
          <Button
            ...
            sx={{
              ...
              textTransform: "none", border: "1px solid rgba(79,70,229,0.28)",
              color: "#4f46e5", "&:hover": { bgcolor: "rgba(79,70,229,0.08)" },
            }}
          >
            Просмотр
          </Button>
        ...
        <Button
          ...
          sx={{
            fontSize: "0.72rem", px: 1.5, py: 0.5, borderRadius: "8px",
            textTransform: "none", bgcolor: "rgba(79,70,229,0.07)",
            color: "#4338ca", "&:hover": { bgcolor: "rgba(79,70,229,0.14)" },
          }}
        >
          Скачать
        </Button>
  ...
  return (
    <Box
      ...
      sx={{
        ...
        px: 2.5, py: 1.75, borderRadius: "14px",
        border: "1px solid",
        borderColor: hovered ? "rgba(79,70,229,0.28)" : "rgba(148,163,184,0.14)",
        bgcolor: hovered ? "rgba(79,70,229,0.03)" : "#fff",
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography variant="body2" fontWeight={600} noWrap
          sx={{ color: "#0f172a", fontSize: "0.87rem" }}>
          {file.filename}
        </Typography>
      </Box>
      ...
      <Box
        sx={{
          ...
          bgcolor: "rgba(79,70,229,0.09)",
          fontSize: "0.65rem", fontWeight: 700, color: "#4f46e5", letterSpacing: "0.05em",
      ...
      <Typography
        ...
        sx={{ flexShrink: 0, width: 72, textAlign: "right", display: { xs: "none", md: "block" } }}
      >
        {formatFileSize(file.file_size)}
      </Typography>
      ...
      <Stack
        ...
        sx={{ flexShrink: 0, opacity: hovered ? 1 : 0, transition: "opacity 0.15s" }}
      >
        {isPreviewable(file.content_type) && (
        ...
        >
          ...
          <Box
            sx={{
              ...
              background:
                "linear-gradient(135deg,rgba(79,70,229,0.04) 0%,rgba(6,182,212,0.04) 100%)",
            }}
          >
            <Stack
          ...
          <Box sx={{ px: { xs: 3, md: 5 }, py: { xs: 3, md: 4 }, flex: 1 }}>
            ...
            {filesQuery.isLoading && (
              <Box py={12} textAlign="center">
                <CircularProgress
                  size={44}
                  sx={{ color: "#4f46e5", "& .MuiCircularProgress-circle": { strokeLinecap: "round" } }}
                />
                <Typography color="text.secondary" mt={2.5} fontSize="0.9rem">
                  Загружаем файлы…
                </Typography>
              </Box>
            )}
            ...
            {!filesQuery.isLoading && !filesQuery.isError && fileCount > 0 && viewMode === "list" && (
              <>
                <Box
                  sx={{ display: "flex", alignItems: "center", gap: 2, px: 2.5, py: 1, mb: 1 }}
                >
                  <Box sx={{ width: 44, flexShrink: 0 }} />
                  {[
                    { label: "Название", flex: 1 },
                  ].map(({ label, flex }) => (
                    <Typography
                      key={label}
                      variant="caption" color="text.secondary"
                      sx={{ flex, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", fontSize: "0.7rem" }}
                    >
                      {label}
                    </Typography>
                  ))}
                  <Typography
                    variant="caption" color="text.secondary"
                    sx={{ fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", fontSize: "0.7rem", display: { xs: "none", sm: "block" }, width: 60 }}
                  >
                    Тип
                  </Typography>
                  <Typography
                    variant="caption" color="text.secondary"
                    sx={{ fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", fontSize: "0.7rem", display: { xs: "none", md: "block" }, width: 72, textAlign: "right" }}
                  >
                    Размер
                  </Typography>
                ...
                <Stack spacing={1}>
                  {files.map((file) => (
                    <FileRowList key={file.id} file={file} token={token ?? ""} />
                  ))}
                </Stack>
              </>
            )}
